...
- **MIL pass pipeline:** `gemma_chat/mil_passes/ct_convert_pipeline.py` starts from `stablehlo_coreml.build_pass_pipeline()` (which inserts the cleanup group `remove_broadcast_tiles`, `fuse_reduce_keep_dims`, dce, `remove_noop_slice_update` before the first `const_elimination`, and the fusion group `replace_decomposed_softmax`, `fuse_attention_to_sdpa`, `fuse_logit_softcap`, `fuse_gelu_erfc` before `fuse_matmul_weight_bias`), then inserts the two project-owned passes: `quantize_const_weights` at index 0 and `collapse_cast_chains` just before the fusion group. It also removes the three problematic passes listed above.
