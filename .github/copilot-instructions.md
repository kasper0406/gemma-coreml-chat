...
The Python test suite in `tests/` covers MIL-pipeline fusions (`test_pipeline_fusions.py`) and multifunction dynamic-shape export (`test_multifunction_dynamic.py`). There is no Swift test target yet — verify Swift changes by `swift build` in `cli/` and by running the CLI against an exported `.mlpackage`.
...
- Three passes are explicitly removed from the pipeline by `build_ct_convert_pass_pipeline()` itself: `add_fp16_cast` (would override fp32 paths and cause NaN), `fuse_layernorm_or_instancenorm`, and `fuse_elementwise_to_batchnorm` (produce incorrect fusions for this model). Callers get the pipeline ready to use — do not re-list the removals at the call site.
...
- **MIL pass pipeline:** `gemma_chat/mil_passes/ct_convert_pipeline.py` starts from `stablehlo_coreml.build_pass_pipeline()` (which inserts its own cleanup, fusion and late-fusion groups — read that function rather than trusting a copy of the list here), then inserts the two project-owned passes: `quantize_const_weights` at index 0 and `collapse_cast_chains` just before the fusion group. It also removes the three problematic passes listed above. `fuse_rmsnorm` is upstream's now, in the late-fusion group, which needs the stablehlo-coreml release *after* 0.1.5.
...
- Always build a **fresh** pipeline object with `build_ct_convert_pass_pipeline()` per conversion — do not mutate stablehlo-coreml’s module-level `DEFAULT_HLO_PIPELINE` in place.
